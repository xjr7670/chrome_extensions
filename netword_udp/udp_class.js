function udp() {
    ...
    this.create = function(callback) {
        _udp.create(this.option, function(soketInfo) {
    ...
    this.receive = function(info) {
        console.log('Recieved data from ' + info.remoteAddress + ':' + info.remotePort);
    },
    ...
    this.setMilticastLoopback = function(enabled, callback) {
        _udp.setMulticastLoopbackMode(this.socketId, enabled, function(code) {
            if (code < 0) {
                this.error(code);
                return false;
            } else {
